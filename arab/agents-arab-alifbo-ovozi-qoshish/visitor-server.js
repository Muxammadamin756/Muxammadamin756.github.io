...
// Harf uchun ovoz yaratish (Google Text-to-Speech)
app.post('/api/generate-audio', async (req, res) => {
    const { text, nom, lang = 'ar-SA' } = req.body;
    
    if (!text || !nom) {
        res.status(400).json({ error: 'text va nom kerak' });
        return;
    }
    ...
    try {
        ...
        res.json({ 
            success: true, 
            message: `Ovoz ${nom} uchun yaratildi`,
            path: `/OVoz/${fileName}`
        });
    } catch (err) {
        console.error('Text-to-Speech xatolik:', err);
        res.status(500).json({ error: 'Ovoz yaratish xatolik' });
    }
});
